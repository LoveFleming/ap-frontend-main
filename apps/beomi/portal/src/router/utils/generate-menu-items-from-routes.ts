...
export function generateMenuItemsFromRoutes(routeList: AppRouteRecordRaw[]) {
	return routeList.reduce<MenuItemType[]>((acc, item) => {
		...
		// DEBUG: log every route item
		console.log("[generateMenuItemsFromRoutes] item:", item);
		...
		if (Array.isArray(item.children) && item.children.length > 0) {
			...
			console.log("[generateMenuItemsFromRoutes] children for", item.path, noIndexRoute);
		...
		console.log("[generateMenuItemsFromRoutes] menuItem generated:", menuItem);
		return [...acc, menuItem];
	}, []);
}
